...
def find_authors_nationalities():
    authors = [
        ...
        for author in Author.objects.exclude(nationality=None)
    ]
    return '\n'.join(authors)
...
def delete_review_by_id(input_id: int):
    deleted_review = Review.objects.get(pk=input_id)
...
def filter_authors_by_birth_year(start_year, end_year):
    filtered_authors = (Author.objects
                        .filter(birth_date__year__range=(start_year, end_year))
                        .order_by('-birth_date'))
    result = [
...
def change_reviewer_name(old_name: str, new_name: str):
    (Review.objects
     .filter(reviewer_name=old_name)
