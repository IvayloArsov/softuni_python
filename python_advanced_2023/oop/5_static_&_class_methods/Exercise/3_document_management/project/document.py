...
class Document:
    ...
    def from_instances(cls, id: int, category: Category, topic: Topic, file_name: str):
        new_instance = cls(id, category.id, topic.id, file_name)
        return new_instance
    ...
    def __repr__(self):
        info_str = f'Document {self.id}: {self.file_name}; category {self.category_id}, topic {self.topic_id}, tags: {", ".join(self.tags)}'
        return info_str
